#!/bin/bash
#SBATCH --job-name=Reproduction_Classification
...
apptainer exec --bind /local,/scratch,/project --nv /project/def-yacineb/reproduction/ollama_env.sif ollama serve > "$SLURM_TMPDIR/ollama.log" 2>&1 &
OLLAMA_PID=$!
